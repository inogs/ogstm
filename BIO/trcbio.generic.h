...
C =============
C
c     DO 1000 jj = ktask+1,jpjm1,ncpu
C
...
c          IF(Tmask(ji,jj,jk) .eq. 1) THEN
c           DO jtr=1, jptra
c               bfm_trn(jtr) = trn(ji,jj,jk,jtr)               
c           END DO

c           call OPA_Input_EcologyDynamics(bfm_trn, jptra,
c    &           tn(ji,jj,jk), sn(ji,jj,jk), xpar(ji,jj,jk))

c           call OPA_reset()

c           call EcologyDynamics()

c           call OPA_Output_EcologyDynamics(bfm_tra, jptra)
c      
c           DO jtr=1, jptra
c               tra(ji,jj,jk,jtr) = bfm_tra(jtr)
c           END DO
cCC            DO jtr=1, jptra
