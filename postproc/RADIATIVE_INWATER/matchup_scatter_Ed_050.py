...
for iline, line in enumerate(READER):  # each line is one simulation
    ...
    for ifile, filename in enumerate(glob.glob(SIM_FOLDER + '/MATCHUP/*.nc')):
        ...
        if ifile == 0:
            ...
            ed_380   = ncin.variables['Ed_380'][depth380<50.,:]
            ed_412   = ncin.variables['Ed_412'][depth412<50.,:]
            ed_490   = ncin.variables['Ed_490'][depth490<50.,:]

        else:

            ed_380 = np.concatenate((ed_380, ncin.variables['Ed_380'][depth380<50.,:]))
            ed_412 = np.concatenate((ed_412, ncin.variables['Ed_412'][depth412<50.,:]))
            ed_490 = np.concatenate((ed_490, ncin.variables['Ed_490'][depth490<50.,:]))
    ...
    OUTDIR = SIM_MAIN_FOLDER + '/PLOTS/SCATTER/Ed_050/' 
