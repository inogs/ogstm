...
def riparto(lenglo,nprocs):
    ...
    #print("rem = ", remainder)
    JP = np.ones((nprocs),np.int32)*mean_value + 2
    JP[ 0] = JP[ 0] - 1
    JP[-1] = JP[-1] - 1
...
def candidate_decompositions(tmask, max_proc_i,max_proc_j,nproc):
    ...
    * Comm_table * a 2d integer array (max_proc_j,max_proc_i)
    ...
    '''
    Needed_procs = np.zeros((max_proc_j,max_proc_i),np.int32)
    Comm_table = np.zeros((max_proc_j,max_proc_i),np.int32)
...
def neighbors(M,nproc,nproci,nprocj):
    ...
    * WEST, EAST, NORTH, SOUTH, NBONDI, NBONDJ *   1d arrays of integers (nproc)
    '''
    J,I = M.nonzero()
    WEST =np.zeros((nproc,),dtype=np.int32)
    SOUTH=np.zeros((nproc,),dtype=np.int32)
    EAST =np.zeros((nproc,),dtype=np.int32)
    NORTH=np.zeros((nproc,),dtype=np.int32)
    NBONDI=np.zeros((nproc,),dtype=np.int32)
    NBONDJ=np.zeros((nproc,),dtype=np.int32)
...
def get_best_decomposition(USED_PROCS, COMMUNICATION, max_nproc, jpiglo, jpjglo):
    ...
    * jpjglo        * integer, global domain size
    ...
    CANDIDATES = np.zeros((0,8),np.int32)
    ...
    for nproc in range(max_nproc,min_nproc,-1):
        ...
        HYP_COMMUNICATION_LINE=np.zeros(nCandidates,dtype=np.int32)
        EFF_COMMUNICATION_LINE=np.zeros(nCandidates,dtype=np.int32)
        ...
        for k in range(nCandidates):
            ...
            load = (xm*ym).max()
            linearray = np.array([nproc, nproci, nprocj, JPI.max(), JPJ.max(), load, EFF_COMMUNICATION_LINE[k], iCandidate],dtype=np.int32,ndmin=2)
