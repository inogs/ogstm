...
def argument():
    ...
    parser.add_argument(   '--isglobal', '-g',
                                action = 'store_true',
                                help = 'Use this flag if you are applying the decomposition to a global map')
...
map_is_global = args.isglobal

from domdec import candidate_decompositions, get_best_decomposition, dump_outfile, plot_decomposition
from bitsea.commons.mask import Mask

TheMask = Mask.from_file(args.maskfile, e3t_var_name="tmask")
tmask = TheMask.mask_at_level(0)
jpjglo, jpiglo = tmask.shape


USED_PROCS, COMMUNICATION = candidate_decompositions(tmask, max_proc_i, max_proc_j, nproc, map_is_global)

choosen_procs, nproci, nprocj =  get_best_decomposition(USED_PROCS, COMMUNICATION, nproc, jpiglo, jpjglo, map_is_global)

dump_outfile(tmask, choosen_procs, nproci, nprocj, map_is_global)

if not args.skipplot:
    fig, ax = plot_decomposition(tmask, nproci, nprocj, map_is_global)
    if fig is not None:
        fig.set_dpi(150)
