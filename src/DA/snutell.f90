...
!        call  MODIFY_NC_3D_byte('conditionsForCorr2.nc','cond_4'//CTYPE, jpiglo,jpjglo,jpk, ICOND4)
       ...
       ! qui dovrei scrivere il RST.after
        ...
        filename(32:34)=IVAR ; tottrnDA = real(CHLfraction* CORR ,4) ; CALL write_restartDA(filename,julian)
        filename(32:34)=NVAR ; tottrnDA = real(  Nfraction* CORRN,4) ; CALL write_restartDA(filename,julian)
        filename(32:34)=PVAR ; tottrnDA = real(  Pfraction* CORRP,4) ; CALL write_restartDA(filename,julian)
        filename(32:34)=CVAR ; tottrnDA = real(  Cfraction* CORRC,4) ; CALL write_restartDA(filename,julian)
        if (CTYPE.eq.'1')then
        filename(32:34)=SVAR ; tottrnDA = real(  Sfraction* CORRS,4) ; CALL write_restartDA(filename,julian)
        endif
