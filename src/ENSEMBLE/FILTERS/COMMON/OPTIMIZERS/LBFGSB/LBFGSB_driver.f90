...
!       of  the driver 
!
      integer,  parameter    :: n = 3, m = n, iprint = -1
...
subroutine LBFGSB_OPT(FG, parameters, l, u, x, fmin, ierror, opt_verbosity)
      
      integer, dimension(2), intent(in) :: parameters
      double precision, dimension(n), intent(in) :: l,u
      double precision, dimension(n), intent(inout) :: x
      double precision, intent(out) :: fmin
      integer, intent(out) :: ierror
      integer, optional, intent(in) :: opt_verbosity
      external :: FG
      
      integer verbosity
      
    if (present(opt_verbosity)) then
...
!     This is the call to the L-BFGS-B code.
         ...
         if (task(1:2) .eq. 'FG') then
            ...
            call FG(x, parameters, f, g, ierror)
            if (ierror/=0) then
                  if (verbosity>0) write(*,*) "problems computing likelihood and gradient at (i,j) = ", parameters
                  return
            end if
...
!     end of loop do while
      ...
      if (task(1:4).eq.'ABNO') then
            if (verbosity>1) write(*,*) "At (i,j) = ", parameters, " abnormal optimization: ", task
      elseif (task(1:5).eq.'ERROR') then
            ierror=-999
            if (verbosity>0) write(*,*) "At (i,j) = ", parameters, " error on optimization input: ", task
      end if
