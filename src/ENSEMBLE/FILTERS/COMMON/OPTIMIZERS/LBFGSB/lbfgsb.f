...
c
c       Minpack2 Library ... lbfgsb_timer
c
c       Linpack Library ... lbfgsb_dcopy, lbfgsb_ddot.
c
c
c     References:
c
...
c     ************
     ...
     +                 iword,nfree,nact,ileave,nenter
      double precision theta,fold,lbfgsb_ddot,dr,rr,tol,
...
c     Compute d=newx-oldx, r=newg-oldg, rr=y'y and dr=y's.
      ...
      do 42 i = 1, n
         r(i) = g(i) - r(i)
  42  continue
      rr = lbfgsb_ddot(n,r,1,r,1)
      if (stp .eq. one) then  
         dr = gd - gdold
         ddum = -gdold
      else
         dr = (gd - gdold)*stp
         call lbfgsb_dscal(n,stp,d,1)
         ddum = -gdold*stp
      endif
...
c                                to be solved by lbfgsb_dtrsl is singular.
c
c     Subprograms called:
c
c       Linpack ... lbfgsb_dtrsl.
c
c
c                           *  *  *
c
...
c     Solve the triangular system
      call lbfgsb_dtrsl(wt,m,col,p(col+1),11,info)
      if (info .ne. 0) return
...
c       solve J^Tp2=p2. 
      call lbfgsb_dtrsl(wt,m,col,p(col+1),01,info)
      if (info .ne. 0) return
...
c
c       Linpack ... lbfgsb_dscal lbfgsb_dcopy, lbfgsb_daxpy.
c
c
c     References:
c
...
c     ************
     ...
     +                 ibp,nleft,ibkmin,iter
      ...
      double precision tu,tl,wmc,wmp,wmw,lbfgsb_ddot,tj,tj0,neggi,sbgnrm
      double precision f2_org
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                   complete the initialization of p for theta not= one.
         call lbfgsb_dscal(col,theta,p(col+1),1)
      endif
...
c     Initialize derivative f2.
      ...
      if (col .gt. 0) then
         ...
         if (info .ne. 0) return
         f2 = f2 - lbfgsb_ddot(col2,v,1,p,1)
      endif
      dtm = -f1/f2
      tsum = zero
      nseg = 1
...
c        temporarily set f1 and f2 for col=0.
      ...
      if (col .gt. 0) then
c                          update c = c + dt*p.
         call lbfgsb_daxpy(col2,dt,p,1,c,1)
...
c           compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'.
         ...
         if (info .ne. 0) return
         wmc = lbfgsb_ddot(col2,c,1,v,1)
         wmp = lbfgsb_ddot(col2,p,1,v,1) 
         wmw = lbfgsb_ddot(col2,wbp,1,v,1)
 
c           update p = p - dibp*wbp. 
         call lbfgsb_daxpy(col2,-dibp,wbp,1,p,1)
...
c       the variables whose breakpoints haven't been reached.
      ...
      call lbfgsb_daxpy(n,tsum,d,1,xcp,1)
...
c       which will be used in computing r = Z'(B(x^c - x) + g).
      ...
      if (col .gt. 0) call lbfgsb_daxpy(col2,dtm,p,1,c,1)
      if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
...
c     Subprograms called:
c
c       Linpack ... lbfgsb_dcopy, lbfgsb_dpofa, lbfgsb_dtrsl.
c
c
c     References:
c       [1] R. H. Byrd, P. Lu, J. Nocedal and C. Zhu, ``A limited
...
c     ************
     ...
     +                 col2,pbegin,pend,dbegin,dend,upcl
      double precision lbfgsb_ddot,temp1,temp2,temp3,temp4
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                          with L' stored in the upper triangle of wn.
      call lbfgsb_dpofa(wn,m2,col,info)
      if (info .ne. 0) then
         info = -1
         return
      endif
c        then form L^-1(-L_a'+R_z') in the (1,2) block.
      col2 = 2*col
      do 71 js = col+1 ,col2
         call lbfgsb_dtrsl(wn,m2,col,wn(1,js),11,info)
  71  continue
...
c        upper triangle of (2,2) block of wn.
      ...
      do 72 is = col+1, col2
         do 74 js = is, col2
         wn(is,js) = wn(is,js) + lbfgsb_ddot(col,wn(1,is),1,wn(1,js),1)
  74        continue
  72     continue

c     Cholesky factorization of (2,2) block of wn.

      call lbfgsb_dpofa(wn(col+1,col+1),m2,col,info)
      if (info .ne. 0) then
         info = -2
         return
      endif
...
c        J' stored in the upper triangle of wt.
      ...
      call lbfgsb_dpofa(wt,m,col,info)
      if (info .ne. 0) then
         info = -3
      endif
...
c
c       Linpack ... lbfgsb_dtrsl, lbfgsb_ddot.
c
c
c                           *  *  *
c
...
c     **********
      ...
      integer          i
      double           precision lbfgsb_ddot,a1,a2
...
c     Subprograms called:
c
c       Linpack ... lbfgsb_dcopy, lbfgsb_ddot.
c
c
c                           *  *  *
c
...
c                                             and the last column of SS:
      pointr = head
      do 51 j = 1, col - 1
         sy(col,j) = lbfgsb_ddot(n,d,1,wy(1,pointr),1)
         ss(j,col) = lbfgsb_ddot(n,ws(1,pointr),1,d,1)
         pointr = mod(pointr,m) + 1
  51  continue
      if (stp .eq. one) then
         ss(col,col) = dtd
...
c     Compute wv:=K^(-1)wv.
      ...
      m2 = 2*m
      col2 = 2*col
      call lbfgsb_dtrsl(wn,m2,col2,wv,11,info)
      if (info .ne. 0) return
      do 25 i = 1, col
         wv(i) = -wv(i)
  25     continue
      call lbfgsb_dtrsl(wn,m2,col2,wv,01,info)
      if (info .ne. 0) return
...
c     Compute d = (1/theta)d + (1/theta**2)Z'W wv.
  ...
  40  continue
      ...
      call lbfgsb_dscal( nsub, one/theta, d, 1 )
c 
c-----------------------------------------------------------------
