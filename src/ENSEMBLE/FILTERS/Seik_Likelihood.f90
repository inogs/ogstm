...
subroutine Seik_Init
    ...
    if (EnsDebug>1) then
        call mpi_barrier(glcomm,ierr)
        if (lwp) write(*,*) "Seik_Init: before Init_Algebra" 
    end if
    ...
    if (EnsDebug>1) then
        call mpi_barrier(glcomm,ierr)
        if (lwp) write(*,*) "Seik_Init: Init_Algebra done" 
    end if
    ...
    if (EnsDebug>1) then
        call mpi_barrier(glcomm,ierr)
        if (lwp) write(*,*) "Seik_Init: before Ens_shared_alloc_base" 
    end if
    ...
    if (EnsDebug>1) then
        call mpi_barrier(glcomm,ierr)
        if (lwp) write(*,*) "Seik_Init done" 
    end if
    
end subroutine

subroutine Seik_Finalize
    integer ierror
...
!                             write(*,*) '-------------------------------------------------------'
            ...
            if (EnsDebug>1) then
                ...
                if (lwp) write(*,*) "Seik analysis: before computing analysis cov" 
            end if
...
!                         write(*,*) ''
        ...
        if (EnsDebug>1) then
            call mpi_barrier(glcomm,ierror)
            if (lwp) write(*,*) "Seik analysis: before changing base" 
        end if
        ...
        if (EnsDebug>1) then
            call mpi_barrier(glcomm,ierror)
            if (lwp) write(*,*) "Seik analysis: before computing ensemble" 
        end if
...
subroutine TTW1T_builder
    ...
    double precision, dimension(0:EnsSize-1,EnsSize-1) :: matrixT
    integer :: indexi
    double precision, dimension(0:EnsSize-1) :: AllWeights
    
    AllWeights=1.0d0/EnsSize
    ...
    if (.false.) then ! true if you want to build T matrix first, false if you want to make it faster.
        ...
        do indexi=1, EnsSize-1
            TTW1T(:,indexi)=matmul(matrixT(:,indexi)/AllWeights,matrixT)
        end do
        ...
        do indexi=1, EnsRankZero
            TTW1T(:,indexi)=-1.0d0
            TTW1T(indexi,indexi)=1.0d0/AllWeights(indexi-1)-1.0d0
        end do
        do indexi=EnsRankZero+1, EnsSize-1
            TTW1T(:,indexi)=-1.0d0
            TTW1T(indexi,indexi)=1.0d0/AllWeights(indexi)-1.0d0
        end do
