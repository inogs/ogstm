...
!     no mpp
      ...
      tra_DIA_2d_IO_HIGH = huge(tra_DIA_2d_IO_HIGH(1,1,1))
       ...
       if (lwp)  allocate(tottrnIO(jpk,jpjglo,jpiglo)) 
       if (lwp) tottrnIO  = huge(tottrnIO(1,1,1)) 
       if (lwp)  allocate(tottrbIO(jpk,jpjglo,jpiglo)) 
       if (lwp) tottrbIO  = huge(tottrbIO(1,1,1)) 
       if (lwp)  allocate(totsnIO (jpk,jpjglo,jpiglo)) 
       if (lwp) totsnIO   = huge(totsnIO(1,1,1))  
       if (lwp)  allocate(tottnIO (jpk,jpjglo,jpiglo)) 
       if (lwp) tottnIO   = huge(tottnIO(1,1,1))  
       if (lwp)  allocate(totvatmIO(jpjglo,jpiglo))    
       if (lwp) totvatmIO = huge(totvatmIO(1,1))  
       if (lwp)  allocate(totempIO(jpjglo,jpiglo))     
       if (lwp) totempIO  = huge(totempIO(1,1))   
       if (lwp)  allocate(totqsrIO(jpjglo,jpiglo))     
       if (lwp) totqsrIO  = huge(totqsrIO(1,1))   
       if (lwp)  allocate(totunIO(jpk,jpjglo,jpiglo))  
       if (lwp) totunIO   = huge(totunIO(1,1,1))  
       if (lwp)  allocate(totvnIO(jpk,jpjglo,jpiglo))  
       if (lwp) totvnIO   = huge(totvnIO(1,1,1))  
       if (lwp)  allocate(totwnIO(jpk,jpjglo,jpiglo))  
       if (lwp) totwnIO   = huge(totwnIO(1,1,1))  
       if (lwp)  allocate(totavtIO(jpk,jpjglo,jpiglo)) 
       if (lwp) totavtIO  = huge(totavtIO(1,1,1)) 
       if (lwp)  allocate(tote3tIO(jpk,jpjglo,jpiglo)) 
       if (lwp) tote3tIO  = huge(tote3tIO(1,1,1)) 
       if (lwp)  allocate(tottmaIO(jpk,jpjglo,jpiglo)) 
       if (lwp) tottmaIO  = huge(tottmaIO(1,1,1)) 
       if (lwp)  allocate(tottrnIO2d(jpjglo,jpiglo))   
