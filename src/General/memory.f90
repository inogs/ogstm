...
! *******************************************************************
      ...
      allocate(e3t(jpi,jpj,jpk)) 
        e3t = huge(e3t(1,1,1))
      allocate(e3t_back(jpi,jpj,jpk)) 
        e3t_back = huge(e3t_back(1,1,1))
      allocate(e3u(jpi,jpj,jpk)) 
        e3u = huge(e3u(1,1,1))
      allocate(e3v(jpi,jpj,jpk)) 
        e3v = huge(e3v(1,1,1))
      allocate(e3w(jpi,jpj,jpk)) 
        e3w = huge(e3w(1,1,1))
      ...
      ahtt = huge(ahtt(1))
       ...
       allocate(e3tdta(jpi,jpj,jpk,2))    
       e3tdta  = huge(e3tdta(1,1,1,1))
       allocate(e3udta(jpi,jpj,jpk,2))    
       e3udta  = huge(e3udta(1,1,1,1))
       allocate(e3vdta(jpi,jpj,jpk,2))    
       e3vdta  = huge(e3vdta(1,1,1,1))
       allocate(e3wdta(jpi,jpj,jpk,2))    
...
!     no mpp
       ...
       allocate(tra_DIA(jpi,jpj,jpk,jptra_dia))            
       tra_DIA= huge(tra_DIA(1,1,1,1))
       allocate(tra_DIA_2d(jpi,jpj,jptra_dia_2d))          
       tra_DIA_2d= huge(tra_DIA_2d(1,1,1))
       if (lwp) allocate(tottrn(jpiglo, jpjglo, jpk))      
        if (lwp) tottrn = huge(tottrn(1,1,1)) 
       if (lwp) allocate(tottrb(jpiglo, jpjglo, jpk))      
        if (lwp) tottrb = huge(tottrb(1,1,1))
