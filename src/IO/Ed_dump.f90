SUBROUTINE Ed_dump(datemean,datefrom,dateTo)
        !---------------------------------------------------------------------
        !
        !                       ROUTINE Ed_dump
        !
        ...
        if (lwp) write(*,*) 'Ed_dump IsBackup = ',IsBackup
        ! ----------------------------------------
        output_file_nc = 'AVE_FREQ_3/ave.20111231-15:30:00.Ed_01.nc'
        call mppsync()
         ...
         DIR='AVE_FREQ_3/'
         n_dumping_cycles = matrix_Ed_row
        ...
        DUMPING_LOOP: DO jv = 1, n_dumping_cycles
                ...
                DO ivar = 1 , nodes!number of variables for each round corresponds to the number of nodes
                    ...
                    ind_var = matrix_Ed(jv,ivar)%var_index

                    IF (counter_var > Ed_dumped_vars) EXIT
                    ...
                    do ji =1 , jpi
                            ...
                            do jj =1 , jpj
                                    ...
                                    do jk =1 , jpk
                                            ...
                                            bufftrn   (ind)= Ed_IO(jk,jj,ji,ind_var)
                                    enddo
                            enddo
                    enddo
        ...
        if (.not.IsBackup) then
           Ed_IO(:,:,:,:) = 0.  !      we reset matrix for new average
        end if
