SUBROUTINE Es_dump(datemean,datefrom,dateTo)
        !---------------------------------------------------------------------
        !
        !                       ROUTINE Es_dump
        !
        ...
        if (lwp) write(*,*) 'Es_dump IsBackup = ',IsBackup
        ! ----------------------------------------
        output_file_nc = 'AVE_FREQ_3/ave.20111231-15:30:00.Es_01.nc'
        call mppsync()
         ...
         DIR='AVE_FREQ_3/'
         n_dumping_cycles = matrix_Es_row
        ...
        DUMPING_LOOP: DO jv = 1, n_dumping_cycles
                ...
                DO ivar = 1 , nodes!number of variables for each round corresponds to the number of nodes
                    ...
                    ind_var = matrix_Es(jv,ivar)%var_index

                    IF (counter_var > Es_dumped_vars) EXIT
                    ...
                    do ji =1 , jpi
                            ...
                            do jj =1 , jpj
                                    ...
                                    do jk =1 , jpk
                                            ...
                                            bufftrn   (ind)= Es_IO(jk,jj,ji,ind_var)
                                    enddo
                            enddo
                    enddo
                ...
                IF (WRITING_RANK_WR)then
                        ...
                        var_to_store = matrix_Es(jv,ind_col)%var_name
                        ind_var      = matrix_Es(jv,ind_col)%var_index
        ...
        if (.not.IsBackup) then
           Es_IO(:,:,:,:) = 0.  !      we reset matrix for new average
        end if
