      SUBROUTINE Esdump(datemean,datefrom,dateTo,FREQ_GROUP)
!     ******************
      USE calendar
      USE myalloc
      USE IO_mem
...
! ! ******* myrank 0 sets indexes of tot matrix where to place its own part
      ...
      tottrnIO (:,totjstart:totjend,totistart:totiend) = Es_DIA_IO_HIGH(:,reljstart:reljend,relistart:reliend,jn_high)
      else
      tottrnIO (:, totjstart:totjend,totistart:totiend) = Es_DIA_IO(:, reljstart:reljend, relistart:reliend,jn) ! diagnostic from reaction model
      endif
       ...
       if (FREQ_GROUP.eq.2) then
             do ji =1, jpi
               ...
               do jj =1 , jpj
                 ...
                 do jk =1 , jpk
                   ind         =  jk + j_contribution + i_contribution
                   buffDIA(ind) = Es_DIA_IO(jk,jj,ji,jn)
                 enddo
               enddo
             enddo
       else
             do ji =1, jpi
               ...
               do jj =1 , jpj
                 ...
                 do jk =1 , jpk
                   ind         =  jk + j_contribution + i_contribution
                   buffDIA(ind) = Es_DIA_IO_HIGH(jk,jj,ji,jn_high)
                 enddo
               enddo
             enddo
       endif
