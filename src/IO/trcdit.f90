        ...
        DUMPING_LOOP: DO jv = 1, n_dumping_cycles
                ...
                DO ivar = 1 , nodes!number of variables for each round corresponds to the number of nodes
                        ...
                        if (FREQ_GROUP.eq.2) then
                                var_to_send = matrix_state_2_counters(jv,ivar)
                                ...
                                do ji =1 , jpi
                                        ...
                                        do jj =1 , jpj
                                                ...
                                                do jk =1 , jpk
                                                        ...
                                                        bufftrn   (ind)= traIO( jk,jj,ji,var_to_send)
                                                enddo
                                        enddo
                                enddo
                        else ! FREQ_GROUP.eq.1
                                var_to_send=matrix_state_1_counters(jv,ivar)
                                IF(var_to_send==-10000) EXIT
                                do ji =1 , jpi
                                        i_contribution= jpk*jpj * (ji - 1 )
                                        do jj =1 , jpj
                                                ...
                                                do jk =1 , jpk
                                                        ind =  jk + j_contribution + i_contribution
                                                        bufftrn   (ind)= traIO_HIGH(jk,jj,ji,var_to_send)
                                                enddo
                                        enddo
                                enddo
                        endif
