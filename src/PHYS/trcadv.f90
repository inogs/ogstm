...
!!!&omp&                un,vn,wn,e2u,e3u,e3v,e1v,e1t,e2t,e3t,zdt )
             DO ji = 1,jpi
                ...
                !big_fact_zbb(jk,jj,ji) = ( abs(zbb(jk,jj,ji)) - zdt*zbb(jk,jj,ji)**2*inv_ev(jk,jj,ji) )!/(e1v(jj,ji)*e2v(jj,ji)*e3t(jk,jj,ji) ) )
                !big_fact_zcc(jk,jj,ji) = ( abs(zcc(jk,jj,ji)) - zdt*zcc(jk,jj,ji)**2*inv_et(jk,jj,ji) )!/(e1t(jj,ji)*e2t(jj,ji)*e3w(jk,jj,ji) ) )
             END DO
          END DO
       END DO
...
! loop unfusion
      DO jj = 2,jpjm1
            ...
            !zkz(jk,jj,ji,A) = fsz(trn(jk,jj,jpi,B),trn(jk-1,jj,jpi,B),zcc(jk,jj,jpi))
            END DO
      END DO
...
!!                advection by antidiffusive mass fluxes and an upstream scheme
             ...
             DO jj = 2,jpjm1
                 DO ji = 2,jpim1
                   junk  = zti(1,jj,ji,A)
                   zkx(1,jj,ji,A) = fsx(junk,zti(1,jj,ji+1,A),zx(jk,jj,ji,A))
                   !zky(1,jj,ji,A) = fsy(junk,zti(1,jj+ 1,ji,A),zy(jk,jj,ji,A))
                 END DO
             END DO
             DO jj = 2,jpjm1
                 DO ji = 2,jpim1
