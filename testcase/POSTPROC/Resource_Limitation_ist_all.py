...
for depth in range(19):
    plt.subplot(4, 5, depth+1)
    rp = np.zeros(4,dtype='float32')
    rn = np.zeros(4,dtype='float32')

    rp[0] =  max(0.,compute_R(mydata,depth,'ruPPY1c','loPPY1c','P1c',alpha_p[0],Qpcmin[0]).mean(0))
    rp[1] =  max(0.,compute_R(mydata,depth,'ruPPY2c','loPPY2c','P2c',alpha_p[1],Qpcmin[1]).mean(0))
    rp[2] =  max(0.,compute_R(mydata,depth,'ruPPY3c','loPPY3c','P3c',alpha_p[2],Qpcmin[2]).mean(0))
    rp[3] =  max(0.,compute_R(mydata,depth,'ruPPY4c','loPPY4c','P4c',alpha_p[3],Qpcmin[3]).mean(0))

    rn[0] =  max(0.,compute_R(mydata,depth,'ruPPY1c','loPPY1c','P1c',alpha_n[0],Qncmin[0]).mean(0))
    rn[1] =  max(0.,compute_R(mydata,depth,'ruPPY2c','loPPY2c','P2c',alpha_n[1],Qncmin[1]).mean(0))
    rn[2] =  max(0.,compute_R(mydata,depth,'ruPPY3c','loPPY3c','P3c',alpha_n[2],Qncmin[2]).mean(0))
    rn[3] =  max(0.,compute_R(mydata,depth,'ruPPY4c','loPPY4c','P4c',alpha_n[3],Qncmin[3]).mean(0))

    ax=plt.plot(mydata[:,depth]['N1p'],(mydata[:,depth]['N3n']+mydata[:,depth]['N4n']))
