...
# Prefisso e cartella di input/output
file_prefix = "RST.20190101-00:00:00."
input_dir = Path("/RESTARTS/")
output_dir = Path("/RESTARTS98/")
...
def process_nc(input_file_path, chiave):
    TRN_chiave = f"TRN{chiave}"
    valori = mapping.get(chiave)
    n_val = len(valori)

    with nc.Dataset(input_file_path, 'r') as src:

        for valore in valori:
            output_filename = f"{file_prefix}{valore}.nc"
            output_path = output_dir / output_filename
            TRN_valore = f"TRN{valore}"
            ...
            with nc.Dataset(output_path, 'w', format=src.file_format) as dst:
                ...
                for name, variable in src.variables.items():
                    if name == TRN_chiave:
                        var = dst.createVariable(TRN_valore, variable.datatype, variable.dimensions)
                        dst[TRN_valore].setncatts(variable.__dict__)
                        
                        # dividi i valori per il numero di file di uscita
                        dst[TRN_valore][:] = src[TRN_chiave][:] / n_val
...
def main():
    for input_file_path in input_dir.glob("*.nc"):
        state_var = input_file_path.name.replace(file_prefix, "").replace(".nc", "")
        if state_var in mapping:
            process_nc(input_file_path, state_var)
