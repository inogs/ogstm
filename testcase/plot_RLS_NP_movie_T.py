...
#define derived type to store informations
    mydtype = [('N1p','f4')    ,('N3n','f4')    ,('N4n','f4'),                                   
               ...
               ('ruPPYn','f4'),('ruPPYp','f4'),('ruBAn','f4'),('ruBAp','f4'), 
               ('dN1p','f4'),('dN3n','f4'),('dN4n','f4'),
    ...
    for var in mydata.dtype.names[18:35]:
       mydata[:,:][var]=M_dia.variables[var].data[:,:].copy()
    for var in mydata.dtype.names[35::]:
    ...
    class SubplotAnimation(animation.TimedAnimation):
        def __init__(self):
            ...
            for d,depth in enumerate(np.arange(0,100,5)):
                ...
                S_lines.append(Line2D([], [], color='Red', lw=4))
                ax[d].add_line(N_P_lines[d])
                ax[d].add_line(S_lines[d])
                ax[d].set_xscale('log')
                ax[d].set_yscale('log')
...
#                      label.set_rotation('vertical') 
        ...
        def _draw_frame(self, framedata):
            ...
            for d,depth in enumerate(np.arange(0,100,5)):
                 ...
                 R1= self.data[i-1:i+1,depth]['dN1p'] + self.data[i-1:i+1,depth]['ruBAp'] + self.data[i-1:i+1,depth]['ruPPYp'] + self.data[i-1:i+1,depth]['N1p'] 
                 R2= self.data[i-1:i+1,depth]['dN3n'] + self.data[i-1:i+1,depth]['dN4n']  + self.data[i-1:i+1,depth]['ruBAn']  + self.data[i-1:i+1,depth]['ruPPYn'] + self.data[i-1:i+1,depth]['N3n'] 

#                R1= self.data[i-1:i+1,depth]['dN1p'] + self.data[i-1:i+1,depth]['ruBAp'] + self.data[i-1:i+1,depth]['ruPPYp'] + rp[:,d].min(0)
#                R2= self.data[i-1:i+1,depth]['dN3n'] + self.data[i-1:i+1,depth]['dN4n']  + self.data[i-1:i+1,depth]['ruBAn']  + self.data[i-1:i+1,depth]['ruPPYn'] + rn[:,d].min(0)
                 self.Source[d].set_data(R1, R2)
